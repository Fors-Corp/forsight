...
/** Pure so it can be reused inside a `useMemo` factory without becoming a dependency. */
...
export const BarChart = React.memo(
  React.forwardRef<HTMLDivElement, BarChartProps>(
    (
      ...
      // Keyed on `series`/`labels`/`stacked` only, not on the cursor's
      // `active` index — hover and keyboard movement re-render this
      // component on every pointer/key event without the totals or the
      // niceScale domain actually changing.
      const { categoryTotals, scale } = React.useMemo(() => {
        ...
        return { categoryTotals: totals, scale: niceScale(0, highest) };
            ...
            >
              {({ width }) => {
                ...
                const lengthOf = (value: number) =>
                  project(value, scale.min, scale.max, plotHeight);
